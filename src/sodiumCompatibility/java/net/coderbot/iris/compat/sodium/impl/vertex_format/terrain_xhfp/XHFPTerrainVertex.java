...
public class XHFPTerrainVertex implements ChunkVertexEncoder, ContextAwareVertexWriter {
	...
	private float midU;
	private float midV;
	private float uSum, vSum;
	private int normal;
	private int tangent;
	private int vertexCount;
	private boolean hasQuadInfo;
	...
	Vector3f normalFallback = new Vector3f();
	QuadViewTerrain.QuadViewTerrainUnsafe quad = new QuadViewTerrain.QuadViewTerrainUnsafe();

	private void computeExtraInfo(long ptr) {
			// FIXME
...
//			int midTexCoord = (midV << 16) | midU;
			...
			NormalHelper.computeFaceNormal(normalFallback, quad);
			int packedNormal = NormI8.pack(normalFallback);
			...
			int tangent = NormalHelper.computeTangent(normalFallback.x, normalFallback.y, normalFallback.z, quad);
